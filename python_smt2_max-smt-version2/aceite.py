...
def ganancia_xMes (i):
    return "ganancia_xmes"+str(i)

def costeCompra_xMes (i):
    return "costeCompra_xmes"+str(i)

def costeAlmac_xMes (i):
    return "costeAlmac_xmes"+str(i)
...
for i in range(numMeses):
   ...
   print(intvar(costeAlmac_xMes(i)))
   print(intvar(costeCompra_xMes(i)))
   print(intvar(ganancia_xMes(i)))
...
for i in range(numMeses):
    ...
    print(addassert(addle("0", ganancia_xMes(i))))
    print(addassert(addle("0", costeCompra_xMes(i))))
    print(addassert(addle("0", costeAlmac_xMes(i))))
#fin dominios
...
for m in range(numMeses):
    for i in range(numAceites):
        g.append(refinado(m,i))
    print(addassert(addeq(ganancia_xMes(m), (addmul(str(valor), addsum(g)  )))))#ganancias


cost = []#almacenar aceite 
for m in range(numMeses-1):
    for i in range(numAceites):
        cost.append(iniMes(m+1, i))
    print(addassert(addeq(costeAlmac_xMes(m), (addmul(str(CA), addsum(cost)  ))   )))

print(addassert(addeq(costeAlmac_xMes(str(numMeses-1)), str(costeAlmacFin))))#almacenar excedente

#coste comprar aceite
costeAc = []
for m in range(numMeses):
    for i in range(numAceites):           
        costeAc.append(addmul(comprado(m, i), str(coste[m][i]) ))
    print(addassert(addeq( costeCompra_xMes(m), (addsum(costeAc)) ) ))
...
cte = 100.0
print(addassertsoft_id_w(addge("beneficio", str(minB*11)) ,"idTotal", str(cte*15)) )

x= range(1,11)
for i in x:
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*i)) , addlt("beneficio",  str(minB*(i+0.1)))) ,"idTotal", str(cte*i))) 
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.1))) ,    addlt("beneficio", str(minB*(i+0.2)))  ) , "idTotal", str(cte*(i+0.1))))
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.2))) ,    addlt("beneficio", str(minB*(i+0.3)))  ) , "idTotal", str(cte*(i+0.2)))) 
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.3))) ,    addlt("beneficio", str(minB*(i+0.4)))  ) , "idTotal", str(cte*(i+0.3)))) 
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.4))) ,    addlt("beneficio", str(minB*(i+0.5)))  ) , "idTotal", str(cte*(i+0.4)))) 
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.5))) ,    addlt("beneficio", str(minB*(i+0.6)))  ) , "idTotal", str(cte*(i+0.5)))) 
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.6))) ,    addlt("beneficio", str(minB*(i+0.7)))  ) , "idTotal",  str(cte*(i+0.6)))) 
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.7))) ,    addlt("beneficio", str(minB*(i+0.8)))  ) ,"idTotal", str(cte*(i+0.7)))) 
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.8))) ,    addlt("beneficio", str(minB*(i+0.9)))  ) ,"idTotal", str(cte*(i+0.8)))) 
    print(addassertsoft_id_w(addand(addge("beneficio", str(minB*(i+0.9))) ,    addlt("beneficio", str(minB*(i+1)))  ) ,"idTotal", str(cte*(i+0.9))))
	

#añadiendo de forma mensual
for i in range(numMeses):
    print(addassertsoft_id_w( addge( addmul(ganancia_xMes(i), str(numMeses)), str(minB))  ,  "idMes"  , str(cte*15*3/4)))
    print(addassertsoft_id_w( addle( addmul("3", costeCompra_xMes(i) ), ganancia_xMes(i))  ,  "idMes"  , str(cte*15*3/4)))    
    print(addassertsoft_id_w( addle( addmul("5", costeAlmac_xMes(i) ), ganancia_xMes(i))  ,  "idMes"  , str(cte*15*3/4)))
